...
import com.thenetcircle.services.media.service.impl.transform.processing.Processing;
...
{
  private static final Logger LOG=LoggerFactory.getLogger(PImageBufferedImageConverter.class);

  private PImageBufferedImageConverter()
  {
  }

  public static PImage toPImage(final BufferedImage image, final String extension)
  {
    if(image==null)
    {
      throw Oops.causedBy("coverting buffered image to PImage howerver te buffered image is empty");
    }
    final long start=Calendar.getInstance().getTimeInMillis();
    final PImage pImage=new PImage(image.getWidth(), image.getHeight(), getColorModel(extension));
  ...
  public static BufferedImage toBufferedImage(final PImage pImage, final String extension)
  {
    final BufferedImage bimage=new BufferedImage(
      pImage.width, pImage.height, getColorModel(extension));
